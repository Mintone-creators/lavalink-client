...
export class LavalinkNode {
  ...
  public resuming: { enabled: boolean; timeout: number | null } = {
    enabled: true,
    timeout: null,
  };
  ...
  constructor(options: LavalinkNodeOptions, manager: NodeManager) {
    ...
    this.options.regions = (this.options.regions || []).map((a) =>
      a.toLowerCase()
    );

    Object.defineProperty(this, NodeSymbol, {
      configurable: true,
      value: true,
    });
  }
  ...
  private async rawRequest(
    ...
    const options: RequestInit & {
      ...
      signal:
        ...
        this.options.requestSignalTimeoutMS > 0
    ...
    if (options.extraQueryUrlParams && options.extraQueryUrlParams?.size > 0) {
      for (const [
        paramKey,
        paramValue,
      ] of options.extraQueryUrlParams.entries()) {
  ...
  public async request(
    endpoint: string,
    modify: ModifyRequest | undefined,
    parseAsText: true
  ): Promise<string>;
  public async request(
    endpoint: string,
  ...
  public async request(
    ...
    if (!this.connected)
      throw new Error(
        "The node is not connected to the Lavalink Server!, Please call node.connect() first!"
      );
    ...
    if (response.status === 404)
      throw new Error(
        `Node Request resulted into an error, request-PATH: ${options.path} | headers: ${JSON.stringify(response.headers)}`
      );
  ...
  public async search(
    ...
    this.NodeManager.LavalinkManager.utils.validateQueryString(
      this,
      Query.query,
      Query.source
    );
    if (Query.source)
      this.NodeManager.LavalinkManager.utils.validateSourceString(
        this,
        Query.source
      );

    if (
      ["bcsearch", "bandcamp"].includes(Query.source) &&
      !this.info.sourceManagers.includes("bandcamp")
    ) {
      throw new Error(
        "Bandcamp Search only works on the player (lavaplayer version < 2.2.0!"
      );
    }
    ...
    if (
      /^https?:\/\//.test(Query.query) ||
      ["http", "https", "link", "uri"].includes(Query.source)
    ) {
      ...
      const prefix =
        Query.source !== "local" ? `${Query.source}:${fttsPrefix}` : "";
    ...
    const res = (await this.request(requestPathAndSearch, (options) => {
      if (
        typeof query === "object" &&
        typeof query.extraQueryUrlParams?.size === "number" &&
        query.extraQueryUrlParams?.size > 0
      ) {
    ...
    if (
      throwOnEmpty === true &&
      (res.loadType === "empty" || !resTracks.length)
    ) {
      if (
        this.NodeManager.LavalinkManager.options?.advancedOptions
          ?.enableDebugEvents
      ) {
        this.NodeManager.LavalinkManager.emit(
          "debug",
          DebugEvents.SearchNothingFound,
          {
            state: "warn",
    ...
    return {
      ...
      playlist:
        res.loadType === "playlist"
          ? {
              ...
              author:
                res.data.info?.author || res.data.pluginInfo?.author || null,
              thumbnail:
                ...
                res.data?.info?.selectedTrack === -1
                  ...
                  : resTracks[res.data?.info?.selectedTrack]
                    ? resTracks[res.data?.info?.selectedTrack]?.info
                        ?.artworkUrl ||
                      resTracks[res.data?.info?.selectedTrack]?.info?.pluginInfo
                        ?.artworkUrl
                    : null) ||
                null,
              uri:
              ...
              selectedTrack:
                ...
                res.data?.info?.selectedTrack === -1
                  ? null
                  : resTracks[res.data?.info?.selectedTrack]
                    ? this.NodeManager.LavalinkManager.utils.buildTrack(
                        resTracks[res.data?.info?.selectedTrack],
                        requestUser
                      )
                    : null,
              duration: resTracks.length
                ? resTracks.reduce(
                    (
                      acc: number,
                      cur: Track & { info: Track["info"] & { length?: number } }
      ...
      tracks: (resTracks.length
        ? resTracks.map((t) =>
            this.NodeManager.LavalinkManager.utils.buildTrack(t, requestUser)
          )
        : []) as Track[],
    };
  }
  ...
  async lavaSearch(
    ...
    const Query =
      this.NodeManager.LavalinkManager.utils.transformLavaSearchQuery(query);
    ...
    if (!this.info.plugins.find((v) => v.name === "lavasearch-plugin"))
      throw new RangeError(
        `there is no lavasearch-plugin available in the lavalink node: ${this.id}`
      );
    if (!this.info.plugins.find((v) => v.name === "lavasrc-plugin"))
      throw new RangeError(
        `there is no lavasrc-plugin available in the lavalink node: ${this.id}`
      );
    ...
    if (
      throwOnEmpty === true &&
      !Object.entries(res).flat().filter(Boolean).length
    ) {
      if (
        this.NodeManager.LavalinkManager.options?.advancedOptions
          ?.enableDebugEvents
        ...
        this.NodeManager.LavalinkManager.emit(
          "debug",
          DebugEvents.LavaSearchNothingFound,
          {
            state: "warn",
    ...
    return {
      tracks:
        res.tracks?.map((v) =>
          this.NodeManager.LavalinkManager.utils.buildTrack(v, requestUser)
        ) || [],
      albums:
        res.albums?.map((v) => ({
          info: v.info,
          pluginInfo:
            (v as unknown as { plugin: unknown })?.plugin || v.pluginInfo,
          tracks: v.tracks.map((v) =>
            this.NodeManager.LavalinkManager.utils.buildTrack(v, requestUser)
          ),
        })) || [],
      artists:
      ...
      pluginInfo:
        res.pluginInfo || (res as unknown as { plugin: unknown })?.plugin,
    };
  }
  ...
  public async updatePlayer(data: PlayerUpdateInfo): Promise<LavalinkPlayer> {
    if (!this.sessionId)
      throw new Error(
        "The Lavalink Node is either not ready, or not up to date!"
      );
      ...
      (r) => {
        ...
        if (data.noReplace) {
          ...
          url.searchParams.append(
            "noReplace",
            data.noReplace === true && typeof data.noReplace === "boolean"
              ? "true"
              : "false"
          );
    ...
    if (
      ...
      this.NodeManager.LavalinkManager.emit(
        "debug",
        DebugEvents.PlayerUpdateSuccess,
        {
          state: "log",
  ...
  public connect(sessionId?: string): void {
    if (this.connected) {
      if (
        ...
        this.NodeManager.LavalinkManager.emit(
          "debug",
          DebugEvents.TryingConnectWhileConnected,
          {
            state: "warn",
    ...
    const headers = {
      ...
      "Client-Name":
        this.NodeManager.LavalinkManager.options.client.username ||
        "lava-client",
    };
  ...
  private heartBeat(): void {
    if (
      ...
      this.NodeManager.LavalinkManager.emit(
        "debug",
        DebugEvents.HeartBeatTriggered,
        {
          state: "log",
    ...
    this.pingTimeout = setTimeout(() => {
      ...
      if (
        ...
        this.NodeManager.LavalinkManager.emit(
          "debug",
          DebugEvents.SocketTerminateHeartBeatTimeout,
          {
            state: "warn",
  ...
  public destroy(
    ...
    const players = this.NodeManager.LavalinkManager.players.filter(
      (p) => p.node.id === this.id
    );
    if (players.size) {
      const enableDebugEvents =
        this.NodeManager.LavalinkManager.options?.advancedOptions
          ?.enableDebugEvents;
      const handlePlayerOperations = () => {
        if (movePlayers) {
          const nodeToMove = Array.from(
            this.NodeManager.leastUsedNodes("playingPlayers")
              ...
              Array.from(players.values()).map((player) =>
                player.changeNode(nodeToMove.options.id).catch((error) => {
                  if (enableDebugEvents) {
                    console.error(
                      `Node > destroy() Failed to move player ${player.guildId}: ${error.message}`
                    );
                  }
                  return player
                    .destroy(
                      error.message ?? DestroyReasons.PlayerChangeNodeFail
                    )
                    .catch((destroyError) => {
                      if (enableDebugEvents) {
                        console.error(
                          `Node > destroy() Failed to destroy player ${player.guildId} after move failure: ${destroyError.message}`
                        );
                      }
                    });
                })
              ...
              Array.from(players.values()).map((player) =>
                player
                  ...
                  .catch((error) => {
                    if (enableDebugEvents) {
                      console.error(
                        `Node > destroy() Failed to destroy player ${player.guildId}: ${error.message}`
                      );
                    }
                  })
              )
          ...
          return Promise.allSettled(
            Array.from(players.values()).map((player) =>
              player
                .destroy(destroyReason || DestroyReasons.NodeDestroy)
                .catch((error) => {
                  if (enableDebugEvents) {
                    console.error(
                      `Node > destroy() Failed to destroy player ${player.guildId}: ${error.message}`
      ...
      handlePlayerOperations().finally(() => {
        ...
        if (deleteNode) {
          ...
          this.NodeManager.emit("disconnect", this, {
            code: 1000,
   ...
   * ```
   */
  public async fetchAllPlayers(): Promise<
    LavalinkPlayer[] | InvalidLavalinkRestRequest | null
  > {
    if (!this.sessionId)
      throw new Error(
        "The Lavalink-Node is either not ready, or not up to date!"
    ...
    return (
      (this.request(`/sessions/${this.sessionId}/players`) as Promise<
        LavalinkPlayer[] | InvalidLavalinkRestRequest | null
      >) || ([] as LavalinkPlayer[])
    );
  }
  ...
  public async fetchPlayer(
    guildId: string
  ): Promise<LavalinkPlayer | InvalidLavalinkRestRequest | null> {
    if (!this.sessionId)
      throw new Error(
        "The Lavalink-Node is either not ready, or not up to date!"
      );
    return this.request(
      `/sessions/${this.sessionId}/players/${guildId}`
    ) as Promise<LavalinkPlayer | InvalidLavalinkRestRequest | null>;
  }
  ...
  public async updateSession(
    resuming?: boolean,
    timeout?: number
  ): Promise<Session | InvalidLavalinkRestRequest | null> {
    if (!this.sessionId)
      throw new Error(
        "the Lavalink-Node is either not ready, or not up to date!"
      );
    ...
    this.resuming = {
      ...
      timeout:
        typeof resuming === "boolean" && resuming === true ? timeout : null,
    };
  ...
  decode = {
    ...
    singleTrack: async (
      ...
      return this.NodeManager.LavalinkManager.utils?.buildTrack(
        (await this.request(
     ...
     * ```
     */
    multipleTracks: async (
      encodeds: Base64[],
      requester: unknown
    ): Promise<Track[]> => {
      if (
        !Array.isArray(encodeds) ||
        !encodeds.every((v) => typeof v === "string" && v.length > 1)
      )
        throw new SyntaxError(
          "You need to provide encodeds, which is an array of base64 strings"
        );
      ...
      return await this.request(`/decodetracks`, (r) => {
        ...
        r.headers!["Content-Type"] = "application/json";
      }).then((r: LavalinkTrack[]) =>
        r.map((track) =>
          this.NodeManager.LavalinkManager.utils.buildTrack(track, requester)
        )
      );
    },
  };
  ...
  lyrics = {
    ...
    subscribe: async (guildId: string): Promise<unknown> => {
      ...
      return await this.request(
        `/sessions/${this.sessionId}/players/${guildId}/lyrics/subscribe`,
        (options) => {
          options.method = "POST";
        }
      );
    ...
    unsubscribe: async (guildId: string): Promise<void> => {
      ...
      return await this.request(
        `/sessions/${this.sessionId}/players/${guildId}/unsubscribe`
      );
    },
  };
  ...
  private syncPlayerData(
    ...
    if (
      ...
      if (typeof data.playerOptions.filters !== "undefined") {
        ...
        if (data.playerOptions.filters.timescale)
          player.filterManager.data.timescale =
            data.playerOptions.filters.timescale;
        if (data.playerOptions.filters.distortion)
          player.filterManager.data.distortion =
            data.playerOptions.filters.distortion;
        if (data.playerOptions.filters.pluginFilters)
          player.filterManager.data.pluginFilters =
            data.playerOptions.filters.pluginFilters;
        if (data.playerOptions.filters.vibrato)
          player.filterManager.data.vibrato =
            data.playerOptions.filters.vibrato;
        ...
        if (data.playerOptions.filters.equalizer)
          player.filterManager.equalizerBands =
            data.playerOptions.filters.equalizer;
        if (data.playerOptions.filters.karaoke)
          player.filterManager.data.karaoke =
            data.playerOptions.filters.karaoke;
        if (data.playerOptions.filters.lowPass)
          player.filterManager.data.lowPass =
            data.playerOptions.filters.lowPass;
        if (data.playerOptions.filters.rotation)
          player.filterManager.data.rotation =
            data.playerOptions.filters.rotation;
        if (data.playerOptions.filters.tremolo)
          player.filterManager.data.tremolo =
            data.playerOptions.filters.tremolo;
    ...
    if (res?.guildId === "string" && typeof res?.voice !== "undefined") {
      ...
      if (
        typeof res?.voice?.connected === "boolean" &&
        res.voice.connected === false
      ) {
  ...
  private async open(): Promise<void> {
      ...
      this.heartBeatInterval = setInterval(() => {
        if (!this.socket)
          return console.error(
            "Node-Heartbeat-Interval - Socket not available - maybe reconnecting?"
          );
    ...
    this.info = await this.fetchInfo().catch(
      (e) => (console.error(e, "ON-OPEN-FETCH"), null)
    );
  ...
  private async message(d: Buffer | string): Promise<void> {
    ...
    switch (payload.op) {
      ...
      case "playerUpdate":
        {
          const player = this.NodeManager.LavalinkManager.getPlayer(
            payload.guildId
          );
          if (!player) {
            if (
              ...
              this.NodeManager.LavalinkManager.emit(
                "debug",
                DebugEvents.PlayerUpdateNoPlayer,
                {
                  state: "error",
          ...
          if (
            ...
            ((player.queue.current?.info?.duration || 0) <=
              (player.LavalinkManager.options.advancedOptions
                .maxFilterFixDuration || 600_000) ||
              (player.queue.current?.info?.uri &&
                isAbsolute(player.queue.current?.info?.uri)))
          ) {
            ...
            if (
              ...
              this.NodeManager.LavalinkManager.emit(
                "debug",
                DebugEvents.PlayerUpdateFilterFixApply,
                {
                  state: "log",
        ...
        if (payload.resumed === true) {
          try {
            ...
            if (
              ...
              this.NodeManager.LavalinkManager.emit(
                "debug",
                DebugEvents.ResumingFetchingError,
                {
                  state: "error",
      ...
      default:
        this.NodeManager.emit(
          "error",
          this,
          new Error(`Unexpected op "${payload.op}" with data`),
          payload
        );
        return;
    }
  ...
  private async handleEvent(
    payload: PlayerEventType & PlayerEvents
  ): Promise<void> {
    ...
    switch (payload.type) {
      ...
      case "WebSocketClosedEvent":
        ...
        break;
      case "SegmentsLoaded":
        this.SponsorBlockSegmentLoaded(
          player,
        ...
        break;
      case "SegmentSkipped":
        this.SponsorBlockSegmentSkipped(
          player,
          player.queue.current as Track,
          payload
        );
        break;
      case "ChaptersLoaded":
        this.SponsorBlockChaptersLoaded(
          player,
          player.queue.current as Track,
          payload
        );
        break;
      case "ChapterStarted":
        this.SponsorBlockChapterStarted(
          player,
          player.queue.current as Track,
          payload
        );
      ...
      default:
        this.NodeManager.emit(
          ...
          this,
          new Error(
            `Node#event unknown event '${(payload as PlayerEventType & PlayerEvents).type}'.`
          ),
  ...
  private async trackStart(
    ...
    if (
      this.NodeManager.LavalinkManager.options?.emitNewSongsOnly === true &&
      ...
      if (
        ...
        this.NodeManager.LavalinkManager.emit(
          "debug",
          DebugEvents.TrackStartNewSongsOnly,
          {
            state: "log",
  ...
  private async trackEnd(
    ...
    if (
      ...
      (player.repeatMode === "off" || player.get("internal_stopPlaying"))
    )
    ...
    if (["loadFailed", "cleanup"].includes(payload.reason)) {
      ...
      if (
        this.NodeManager.LavalinkManager.options.autoSkip &&
        player.queue.current
      ) {
    ...
    if (player.repeatMode !== "track" || player.get("internal_skipped"))
      ...
      if (player.queue.previous.length > player.queue.options.maxPreviousTracks)
        player.queue.previous.splice(
          player.queue.options.maxPreviousTracks,
  ...
  private async trackStuck(
    ...
    if (
      ...
      const oldTimestamps = (
        (player.get("internal_erroredTracksTimestamps") as number[]) || []
      ).filter(
        (v) =>
          Date.now() - v <
          this.NodeManager.LavalinkManager.options.playerOptions
      ...
      player.set("internal_erroredTracksTimestamps", [
        ...oldTimestamps,
        Date.now(),
      ]);
      if (
        ...
        if (
          ...
          this.NodeManager.LavalinkManager.emit(
            "debug",
            DebugEvents.TrackStuckMaxTracksErroredPerTime,
            {
              state: "log",
    ...
    this.NodeManager.LavalinkManager.emit(
      "trackStuck",
      player,
      track || this.getTrackOfPayload(payload),
      payload
    );
    // If there are no songs in the queue
    if (
    ...
    if (
      ...
      if (
        ...
        if (
          ...
          this.NodeManager.LavalinkManager.emit(
            "debug",
            DebugEvents.TrackErrorMaxTracksErroredPerTime,
            {
              state: "log",
  ...
  public async getSponsorBlock(player: Player): Promise<SponsorBlockSegment[]> {
    ...
    if (!this.info.plugins.find((v) => v.name === "sponsorblock-plugin"))
      throw new RangeError(
        `there is no sponsorblock-plugin available in the lavalink node: ${this.id}`
      );
    // do the request
    return (await this.request(
  ...
  public async setSponsorBlock(
    ...
    if (!segments.length)
      throw new RangeError(
        "No Segments provided. Did you ment to use 'deleteSponsorBlock'?"
      );
    // a not valid segment
  ...
  public async deleteSponsorBlock(player: Player): Promise<void> {
    ...
    if (
      ...
      this.NodeManager.LavalinkManager.emit(
        "debug",
        DebugEvents.DeleteSponsorBlock,
        {
          state: "log",
  ...
  private async queueEnd(
    ...
    if (
      typeof this.NodeManager.LavalinkManager.options?.playerOptions
        ?.onEmptyQueue?.autoPlayFunction === "function" &&
      ...
      const previousAutoplayTime = player.get(
        "internal_previousautoplay"
      ) as number;
      const duration = previousAutoplayTime
        ? Date.now() - previousAutoplayTime
        : 0;
      if (
        !duration ||
        duration >
        ...
        await this.NodeManager.LavalinkManager.options?.playerOptions?.onEmptyQueue?.autoPlayFunction(
          player,
          track
        );
        ...
        if (player.queue.tracks.length > 0) await queueTrackEnd(player);
          ...
          this.NodeManager.LavalinkManager.emit(
            "debug",
            DebugEvents.AutoplayNoSongsAdded,
            {
              state: "warn",
        ...
        if (
          ...
          this.NodeManager.LavalinkManager.emit(
            "debug",
            DebugEvents.AutoplayThresholdSpamLimiter,
            {
              state: "warn",
    ...
    if (
      typeof this.NodeManager.LavalinkManager.options.playerOptions
        ?.onEmptyQueue?.destroyAfterMs === "number" &&
      !isNaN(
        this.NodeManager.LavalinkManager.options.playerOptions.onEmptyQueue
          ?.destroyAfterMs
      ) &&
      this.NodeManager.LavalinkManager.options.playerOptions.onEmptyQueue
        ?.destroyAfterMs >= 0
      ...
      if (
        ...
        if (
          ...
          this.NodeManager.LavalinkManager.emit(
            "debug",
            DebugEvents.TriggerQueueEmptyInterval,
            {
              state: "log",
        ...
        player.set(
          ...
          setTimeout(() => {
            player.set("internal_queueempty", undefined);
            if (player.queue.current) {
              return this.NodeManager.LavalinkManager.emit(
